...
inline bool FastPID::setCoefficients(float kp, float ki, float kd, float hz) {
    _p = floatToParam(kp);
...
inline bool FastPID::setOutputConfig(int bits, bool sign) {
    // Set output bits
    if (bits > 16 || bits < 1) {
        setCfgErr();
    }
...
inline bool FastPID::setOutputRange(int16_t min, int16_t max)
{
    if (min >= max) {
        setCfgErr();
        return ! _cfg_err;
...
inline bool FastPID::configure(float kp, float ki, float kd, float hz, int bits, bool sign) {
    clear();
...
inline uint32_t FastPID::floatToParam(float in) {
...
inline int16_t FastPID::step(int16_t sp, int16_t fb) {
